...
def save_models(output_dir, models):
    ...
    for k, v in models.transformer.state_dict().items():
    ...
    def forward(
        ...
        noise_pred = self.transformer(
            ...
            controlnet_block_samples=[
                sample.to(dtype=hidden_states.dtype) for sample in controlnet_block_samples
            ] if controlnet_block_samples is not None else None,
            controlnet_single_block_samples=[
                sample.to(dtype=hidden_states.dtype) for sample in controlnet_single_block_samples
...
def main(args):
    ...
    params_to_clip = [p for p in transformer.parameters() if p.requires_grad] + list(controlnet.parameters())
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(union_model):
                ...
                control_input = FluxPipeline._pack_latents(
                    control_input,
