...
from diffusers.data import FLUXICImageDataset, flux_collate_fn
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to use independent coordinates."
    )
...
def main(args):
    ...
    # make img_ids and txt_ids
    vae_scale_factor = 8
    h = args.resolution[0] // vae_scale_factor
    w = args.resolution[1] // vae_scale_factor
    if args.independent_coordinates:
        latent_image_ids = []
        for i in range(args.num_images[0]):
            row_image_ids = []
            for j in range(args.num_images[1]):
                latent_ids = torch.zeros(h // 2, w // 2, 3)
                latent_ids[..., 1] = latent_ids[..., 1] + torch.arange(h // 2)[:, None]
                latent_ids[..., 2] = latent_ids[..., 2] + torch.arange(w // 2)[None, :]
                latent_ids[..., 0] = i * args.num_images[1] + j
                row_image_ids.append(latent_ids)
            row_image_ids = torch.cat(row_image_ids, dim=1)
            latent_image_ids.append(row_image_ids)
        latent_image_ids = torch.cat(latent_image_ids, dim=0)
        latent_image_ids = latent_image_ids.reshape(-1, latent_image_ids.shape[-1])
        latent_image_ids = latent_image_ids.to(device=accelerator.device, dtype=weight_dtype)
    else:
        latent_image_ids = FluxPipeline._prepare_latent_image_ids(
            args.train_batch_size,
            h // 2 * args.num_images[0],
            w // 2 * args.num_images[1],
            accelerator.device,
            weight_dtype,
        )
    ...
    # make fill mask
    mask_zeros = torch.zeros([args.train_batch_size, args.resolution[0], args.resolution[1]])
    mask_ones = torch.ones([args.train_batch_size, args.resolution[0], args.resolution[1]])
    mask = []
    for i in range(args.num_images[0]):
        row_mask = []
        for j in range(args.num_images[1]):
            if i == 0 and j == 0:
                row_mask.append(mask_zeros)
            else:
                row_mask.append(mask_ones)
        row_mask = torch.cat(row_mask, dim=-1)
        mask.append(row_mask)
    mask = torch.cat(mask, dim=-2)
    mask = mask.view(
        args.train_batch_size, h * args.num_images[0], vae_scale_factor, w * args.num_images[1], vae_scale_factor
    )  # batch_size, height, 8, width, 8
    mask = mask.permute(0, 2, 4, 1, 3)  # batch_size, 8, 8, height, width
    mask = mask.reshape(
        args.train_batch_size, vae_scale_factor * vae_scale_factor, h * args.num_images[0], w * args.num_images[1]
    ...
    train_dataset = FLUXICImageDataset(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                model_input = batch["pixel_values"].to(weight_dtype)
                refer_input = model_input.chunk(2, dim=-1)[0].detach()
                ...
                packed_noisy_model_input = torch.cat([packed_noisy_model_input, packed_mask.detach()], dim=-1)

                # Predict the noise residual
                model_pred = transformer(
                    hidden_states=packed_noisy_model_input.detach(),
                    timestep=timesteps,
                    encoder_hidden_states=batch["prompt_embeds"].detach(),
                    pooled_projections=batch["pooled_prompt_embeds"].detach(),
