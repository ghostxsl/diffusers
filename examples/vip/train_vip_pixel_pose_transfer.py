...
from diffusers.data import PoseTransDataset, animate_collate_fn
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for model in models:
                ...
                if isinstance(model, ReferenceTextureModel):
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(model, ReferenceTextureModel):
                load_model = ReferenceTextureModel.from_pretrained(input_dir, subfolder="referencenet")
            else:
                load_model = VIPUNet2DConditionModel.from_pretrained(input_dir, subfolder="unet")
    ...
    if args.pretrained_model_path:
        ...
        unet = VIPUNet2DConditionModel.from_pretrained(args.pretrained_model_path, subfolder="unet")
        referencenet = ReferenceTextureModel.from_pretrained(args.pretrained_model_path, subfolder="referencenet")
    else:
        ...
        if args.unet_model_path:
            ...
            unet = VIPUNet2DConditionModel.from_pretrained(args.unet_model_path)
        else:
        ...
        if args.referencenet_model_path:
            ...
            referencenet = ReferenceTextureModel.from_pretrained(args.referencenet_model_path)
        else:
    ...
    params_to_optimize = list(referencenet.parameters()) + unet_params
    ...
    train_dataset = PoseTransDataset(
    ...
    unet, referencenet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, referencenet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, referencenet):
                ...
                reference_latents = batch["reference_pixel_values"].to(
                    dtype=weight_dtype) * batch["uncond"].to(dtype=weight_dtype)
                referencenet(reference_latents, timesteps)
                reference_control_reader.update(
                    accelerator.unwrap_model(referencenet), dtype=weight_dtype)
