...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to controlnetxs model.",
    )
    parser.add_argument(
...
def main(args):
    ...
    # TODO: Get the Zero image embedding for conditioning
    image_embeds = image_encoder(
        torch.zeros([1, 3, 224, 224], dtype=weight_dtype, device=accelerator.device),
        output_hidden_states=True).hidden_states[-2]
    image_embeds = torch.zeros_like(image_embeds)
    image_embeds = image_embeds.repeat_interleave(repeats=args.train_batch_size, dim=0)
    del image_encoder
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                model_pred = controlnet(
                    ...
                    added_cond_kwargs={'image_embeds': image_embeds.detach()},
                    return_dict=False,
                )[0]
