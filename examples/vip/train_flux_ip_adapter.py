...
def save_ip_adapter_state_dict(save_dir, state_dict, dtype=torch.float32):
    ...
    for k, v in state_dict.items():
        if "processor" in k or "pooled_im_embedder" in k or "x_embedder" in k:
...
def main(args):
    ...
    if exists(join(args.pretrained_model_name_or_path, "empty_prompt_embeds")):
        ...
        pooled_prompt_embeds = empty_prompt_embeds["pooled_prompt_embeds"].repeat_interleave(args.train_batch_size,
                                                                                             dim=0)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                model_pred = transformer(
                    hidden_states=packed_noisy_model_input if not alter_x_embedder else \
                        torch.cat([packed_noisy_model_input, packed_refer_input], dim=2),
                    timestep=timesteps,
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        save_ip_adapter_state_dict(
                            join(save_path, "ip_adapter"), unwrap_model(transformer).state_dict())
                        if args.use_ema:
                            save_ip_adapter_state_dict(join(save_path, "ema_ip_adapter"), ema_transformer.state_dict)
    ...
    if accelerator.is_main_process:
        ...
        save_ip_adapter_state_dict(join(args.output_dir, "ip_adapter"), transformer.state_dict())
        if args.use_ema:
            save_ip_adapter_state_dict(join(args.output_dir, "ema_ip_adapter"), ema_transformer.state_dict)
