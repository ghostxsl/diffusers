...
from diffusers.models.dinov2 import dinov2_vitl14_reg
...
from diffusers.data import ImageVariationDataset, flux_collate_fn, ModelEMA
...
def save_ip_adapter_state_dict(save_dir, state_dict, alter_x_embedder=False, dtype=torch.float32):
    if not exists(save_dir):
        os.makedirs(save_dir, exist_ok=True)

    ip_adapter_state_dict = {}
    for k, v in state_dict.items():
        if "processor" in k or "pooled_im_embedder" in k:
            ip_adapter_state_dict[k] = v.to(dtype)
        elif alter_x_embedder and "x_embedder" in k:
            ip_adapter_state_dict[k] = v.to(dtype)
        elif k.startswith("ip_image_proj"):
            ip_adapter_state_dict[k] = v.to(dtype)
...
def main(args):
    ...
    # Load the empty prompt embeddings
    ...
    if args.use_dinov2:
        image_encoder = Dinov2Model.from_pretrained(args.image_encoder_model_path)
        # image_encoder = dinov2_vitl14_reg(
        #     pretrained=join(args.image_encoder_model_path, "dinov2_vitl14_reg4_pretrain.pth"))
    ...
    transformer._init_ip_adapter_plus(
        num_image_text_embeds=args.num_image_text_embeds,
    ...
    if args.use_self_attn:
        ...
        added_seq_len = feature_extractor.size['shortest_edge'] // patch_size
        added_seq_len = added_seq_len * added_seq_len
        added_ids = torch.zeros(added_seq_len, 3)
        added_ids[:, 0] = 1
        added_ids = added_ids.to(device=accelerator.device, dtype=weight_dtype)
    ...
    if args.use_ema and accelerator.is_main_process:
        ema_transformer = ModelEMA(unwrap_model(transformer), args.ema_decay)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                noisy_model_input = FluxPipeline._pack_latents(
                    noisy_model_input,
                ...
                # pooled_image_embeds = None
                if args.use_dinov2:
                    # image_embeds = image_encoder.get_intermediate_layers(
                    #     batch["reference_image"].to(dtype=weight_dtype), norm=False)[-1]
                    image_embeds = image_encoder(
                        batch["reference_image"].to(dtype=weight_dtype),
                        output_hidden_states=True).hidden_states[-1][:, 1:]
                    # pooled_image_embeds = output_encoder.pooled_output
                    # pooled_image_embeds = pooled_image_embeds * batch["uncond"].squeeze(1).to(dtype=weight_dtype)
                else:
                    # clip
                    image_embeds = image_encoder(
                        batch["reference_image"].to(dtype=weight_dtype),
                ...
                model_pred = transformer(
                    hidden_states=noisy_model_input if not alter_x_embedder else \
                        torch.cat([noisy_model_input, model_input], dim=2),
                    timestep=timesteps,
                    ...
                    added_ids=added_ids if added_ids is None else added_ids.detach(),
                    return_dict=False,
                )[0]
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        save_ip_adapter_state_dict(
                            join(save_path, "ip_adapter"), unwrap_model(transformer).state_dict(), alter_x_embedder=args.alter_x_embedder)
                        if args.use_ema:
                            save_ip_adapter_state_dict(join(save_path, "ema_ip_adapter"), ema_transformer.state_dict, alter_x_embedder=args.alter_x_embedder)
    ...
    if accelerator.is_main_process:
        ...
        save_ip_adapter_state_dict(join(args.output_dir, "ip_adapter"), transformer.state_dict(), alter_x_embedder=args.alter_x_embedder)
        if args.use_ema:
            save_ip_adapter_state_dict(join(args.output_dir, "ema_ip_adapter"), ema_transformer.state_dict, alter_x_embedder=args.alter_x_embedder)
