...
from diffusers.data import ImageVariationDataset, kolors_collate_fn, ModelEMA
...
def main(args):
    ...
    # Load the image encoder "clip-336"
    ...
    unet.register_to_config(encoder_hid_dim_type="text_proj")
    unet.register_to_config(encoder_hid_dim=image_encoder.config.hidden_size)
    unet.encoder_hid_proj = torch.nn.Linear(
        unet.config.encoder_hid_dim, unet.config.cross_attention_dim)
    unet.register_to_config(projection_class_embeddings_input_dim=image_encoder.config.hidden_size)
    unet.add_embedding = TimestepEmbedding(unet.config.projection_class_embeddings_input_dim, 1280)
    ...
    if args.use_ema and accelerator.is_main_process:
        ema_unet = ModelEMA(accelerator.unwrap_model(unet), args.ema_decay)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                model_pred = unet(
                    ...
                    added_cond_kwargs={"pooled_image_embeds": pooled_image_embeds},
                    return_dict=False,
                )[0]
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if args.use_ema:
                            accelerator.unwrap_model(unet).save_config(join(save_path, "ema"))
                            ema_unet.save_model(join(save_path, "ema"))
    ...
    if accelerator.is_main_process:
        ...
        if args.use_ema:
            accelerator.unwrap_model(unet).save_config(join(args.output_dir, "ema"))
            ema_unet.save_model(join(args.output_dir, "ema"))
