...
def save_models(output_dir, model):
    ...
    for k, v in model.transformer.state_dict().items():
        if "norm_q" in k or "norm_k" in k or "x_embedder" in k:
            iv_state_dict[k] = v
...
def main(args):
    ...
    image_encoder = SiglipVisionModel.from_pretrained(args.redux_model_path, subfolder="image_encoder")
    feature_extractor = SiglipImageProcessor.from_pretrained(args.redux_model_path, subfolder="feature_extractor")
    ...
    if args.image_embedder_model_path:
        ...
        image_embedder = ReduxImageEncoder.from_pretrained(args.redux_model_path, subfolder="image_embedder")
    ...
    params_to_clip = [p for p in transformer.parameters() if p.requires_grad] + \
        [p for p in image_embedder.parameters() if p.requires_grad]
