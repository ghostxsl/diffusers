...
from diffusers.models.vip.transformer_wan import WanTransformer3DModel
...
def main(args):
    ...
    image_processor = CLIPImageProcessor.from_pretrained(args.pretrained_model_name_or_path, subfolder="image_processor")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                model_input = encode_vae_image(pixel_values, vae, latents_mean, latents_std, weight_dtype)
                ...
                image_embeds = image_encoder(
                    batch["reference_image"], output_hidden_states=True).hidden_states[-2]

                num_latent_frames, latent_height, latent_width = noisy_model_input.shape[2:]
                num_frames = (num_latent_frames - 1) * vae_scale_factor_temporal + 1
                mask_lat_size = torch.ones(bsz, 1, num_frames, latent_height, latent_width)
                mask_lat_size[:, :, 1: -1] = 0
                first_frame_mask = mask_lat_size[:, :, 0:1]
                first_frame_mask = torch.repeat_interleave(first_frame_mask, dim=2,
                                                           repeats=vae_scale_factor_temporal)
                mask_lat_size = torch.concat([first_frame_mask, mask_lat_size[:, :, 1:, :]], dim=2)
                mask_lat_size = mask_lat_size.view(bsz, -1, vae_scale_factor_temporal, latent_height,
                                                   latent_width)
                mask_lat_size = mask_lat_size.transpose(1, 2)
                mask_lat_size = mask_lat_size.to(accelerator.device, dtype=weight_dtype)

                condition = encode_vae_image(batch["reference_pixel_values"], vae, latents_mean, latents_std, weight_dtype)
