...
from diffusers.data import FluxTextPTDataset, flux_collate_fn
...
def main(args):
    ...
    train_dataset = FluxTextPTDataset(
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                packed_noisy_model_input = FluxPipeline._pack_latents(
                    noisy_model_input.to(dtype=weight_dtype),
                ...
                encoder_hidden_states = (batch["prompt_embeds"] * batch["uncond"]).to(dtype=weight_dtype)
                pooled_projections = (batch["pooled_prompt_embeds"] * batch["uncond"].squeeze(-1)).to(dtype=weight_dtype)
