...
from diffusers.data import ControlNetDataset, flux_collate_fn
...
def main(args):
    ...
    train_dataset = ControlNetDataset(
        ...
        tokenizer_clip=tokenizer_one if not args.use_empty_prompt else None,
        tokenizer_t5=tokenizer_two if not args.use_empty_prompt else None,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_controlnet):
                ...
                if not args.use_empty_prompt:
                    with torch.no_grad():
                        pooled_prompt_embeds = text_encoder_one(
                            batch["pooled_input_ids"], output_hidden_states=False).pooler_output
                        prompt_embeds = text_encoder_two(
                            batch["input_ids"], output_hidden_states=False)[0]

                control_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype)

                model_pred = flux_controlnet(
                    base_model=flux_transformer,
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        model = unwrap_model(flux_controlnet)
