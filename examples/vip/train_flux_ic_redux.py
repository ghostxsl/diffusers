...
def save_models(output_dir, model):
    ...
    # save image_embedder
    model.image_embedder.save_pretrained(join(output_dir, "image_embedder"))

    # save state_dict
    iv_state_dict = {}
    for k, v in model.transformer.state_dict().items():
        if "norm_q" in k or "norm_k" in k:
            iv_state_dict[k] = v
...
class UnionModel(torch.nn.Module):
    def __init__(self, transformer, image_embedder):
        super().__init__()
    ...
    def forward(
            ...
            **kwargs):
        image_embeds = self.image_embedder(image_embeds)[0]
...
def main(args):
    ...
    prompt_embeds = torch.zeros([args.train_batch_size, args.max_sequence_length, 4096])
    prompt_embeds = prompt_embeds.to(accelerator.device, dtype=weight_dtype)

    pooled_prompt_embeds = torch.zeros([args.train_batch_size, 768])
    pooled_prompt_embeds = pooled_prompt_embeds.to(accelerator.device, dtype=weight_dtype)

    # Load the image encoder
    image_encoder = SiglipVisionModel.from_pretrained(args.pretrained_redux_model_path, subfolder="image_encoder")
    feature_extractor = SiglipImageProcessor.from_pretrained(args.pretrained_redux_model_path, subfolder="feature_extractor")
    image_encoder.requires_grad_(False)
    image_encoder.to(accelerator.device, dtype=weight_dtype)

    # Load ReduxImageEncoder
    if args.image_embedder_model_path:
        image_embedder = ReduxImageEncoder.from_pretrained(args.image_embedder_model_path)
    else:
        image_embedder = ReduxImageEncoder.from_pretrained(args.pretrained_redux_model_path, subfolder="image_embedder")
    ...
    transformer_lora_config = LoraConfig(
        ...
        target_modules=[
            ...
            "norm1.linear", "norm.linear", "norm_out.linear",
            "proj_mlp", "proj_out",
        ],
    )
    ...
    transformer._init_fill_x_embedder(state_dict=state_dict)
    ...
    text_ids = torch.zeros(args.max_sequence_length + 729, 3)
    text_ids = text_ids.to(device=accelerator.device, dtype=weight_dtype)

    mask = torch.cat([
        torch.zeros([args.train_batch_size, args.resolution, args.resolution]),
        torch.ones([args.train_batch_size, args.resolution, args.resolution])], dim=-1)
    mask = mask.view(
        args.train_batch_size, latent_hw, vae_scale_factor, latent_hw * 2, vae_scale_factor
    )  # batch_size, height, 8, width, 8
    mask = mask.permute(0, 2, 4, 1, 3)  # batch_size, 8, 8, height, width
    mask = mask.reshape(
        args.train_batch_size, vae_scale_factor * vae_scale_factor, latent_hw, latent_hw * 2
    )  # batch_size, 8*8, height, width
    packed_mask = FluxPipeline._pack_latents(
        mask,
    ...
    params_to_clip = [p for p in transformer.parameters() if p.requires_grad] + list(image_embedder.parameters())
    ...
    union_model = UnionModel(transformer, image_embedder)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(union_model):
                ...
                model_input = torch.cat([refer_input, model_input], dim=3)
                ...
                masked_image = torch.cat([refer_input, torch.zeros_like(refer_input)], dim=3)

                noisy_model_input = torch.cat([noisy_model_input, masked_image], dim=1)
                ...
                control_input = torch.cat([torch.zeros_like(control_input), control_input], dim=3)
                packed_control_input = FluxPipeline._pack_latents(
                    control_input,
                    batch_size=control_input.shape[0],
                    num_channels_latents=control_input.shape[1],
                    height=control_input.shape[2],
                    width=control_input.shape[3],
                )

                packed_noisy_model_input = torch.cat(
                    [packed_noisy_model_input, packed_mask.detach(), packed_control_input], dim=-1)

                image_embeds = image_encoder(
                    batch["reference_image"].to(dtype=weight_dtype)).last_hidden_state
