...
def save_models(output_dir, transformer):
    ...
    for k, v in transformer.state_dict().items():
        if "x_embedder" in k:
            x_embedder_state_dict[k] = v
    torch.save(x_embedder_state_dict, join(output_dir, "x_embedder.bin"))
    logger.info(f"Saved x_embedder weight to {output_dir}.")
...
def main(args):
    ...
    # Load IP Adapter
    state_dict = None
    if args.x_embedder_model_path is not None:
        state_dict = torch.load(args.x_embedder_model_path, map_location='cpu')
    transformer._init_concat_x_embedder(state_dict)
    ...
    print(f"Training params: {num_params / 1000 / 1000} M")
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                model_pred = transformer(
                    hidden_states=torch.cat([packed_noisy_model_input, refer_input], dim=2),
                    timestep=timesteps,
