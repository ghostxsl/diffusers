...
def load_weights_to_transformer(old_transformer, new_transformer):
    # time_text_embed
    new_transformer.time_text_embed.timestep_embedder.load_state_dict(
        old_transformer.time_text_embed.timestep_embedder.state_dict())
    # x_embedder
    new_transformer.x_embedder.load_state_dict(old_transformer.x_embedder.state_dict())
    # norm_out
    new_transformer.norm_out.load_state_dict(old_transformer.norm_out.state_dict())
    # proj_out
    new_transformer.proj_out.load_state_dict(old_transformer.proj_out.state_dict())

    # transformer_blocks
    for i, block in enumerate(new_transformer.transformer_blocks):
        block.load_state_dict(old_transformer.transformer_blocks[i].state_dict())

    # single_transformer_blocks
    for i, block in enumerate(new_transformer.single_transformer_blocks):
        block.load_state_dict(old_transformer.single_transformer_blocks[i].state_dict())
...
def main(args):
    ...
    if args.transformer_model_name_or_path:
        ...
        pretrained_transformer = FluxTransformer2DModel.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="transformer", variant=args.variant)
        ...
        load_weights_to_transformer(pretrained_transformer, transformer)
        del pretrained_transformer
        free_memory()

    if args.alter_x_embedder:
        # concat reference image
        x_embedder_weights = transformer.x_embedder.weight.data
        x_embedder_bias = transformer.x_embedder.bias.data

        transformer.x_embedder = torch.nn.Linear(
            transformer.config.in_channels * 2, transformer.inner_dim)
        for p in transformer.x_embedder.parameters():
            torch.nn.init.zeros_(p)

        transformer.x_embedder.weight.data[:, :transformer.config.in_channels] = x_embedder_weights
        transformer.x_embedder.bias.data = x_embedder_bias
        transformer.x_embedder = transformer.x_embedder.to(device=transformer.device, dtype=transformer.dtype)
        transformer.register_to_config(in_channels=transformer.config.in_channels * 2)
