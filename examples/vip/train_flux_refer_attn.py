...
from diffusers.data import ImageVariationDataset, flux_collate_fn
from diffusers.pipelines.flux.pipeline_flux import calculate_shift
from diffusers.models.vip.transformer_flux import IPAdapterFluxSelfAttnProcessor2_0
from diffusers.models.vip.refernet_flux import ReferAttnProcessor2_0
...
def torch_attn_dfs(model, prefix=""):
    ...
    for name, child in model.named_children():
        if isinstance(child, IPAdapterFluxSelfAttnProcessor2_0):
            result.append([prefix + f".{name}", child])
        elif isinstance(child, ReferAttnProcessor2_0):
            result.append([prefix + f".{name}", child])
        else:
...
def refer_attn_update(transformer, refernet):
    reader_attn_modules = torch_attn_dfs(transformer)
    writer_attn_modules = torch_attn_dfs(refernet)
...
def save_models(output_dir, model):
    ...
    # save refernet
    model.refernet.save_pretrained(join(output_dir, "refernet"))
...
class UnionModel(torch.nn.Module):
    def __init__(self, transformer, refernet):
        super().__init__()
        self.transformer = transformer
        self.refernet = refernet
    ...
    def forward(
        ...
        self.refernet(
            ...
            encoder_hidden_states=reference_image,
            timestep=timestep,
            img_ids=img_ids,
            added_ids=added_ids,
        )
        refer_attn_update(self.transformer, self.refernet)
...
def main(args):
    ...
    # Load refer attn-net
    if args.refer_attn_model_path:
        logger.info("Loading existing refernet weights")
        refernet = FluxReferNetModel.from_pretrained(args.refer_attn_model_path)
    else:
        logger.info("Initializing refernet weights from transformer")
        refernet = FluxReferNetModel.from_transformer(
            transformer,
    ...
    added_ids = latent_image_ids.clone()
    added_ids[:, 0] = 1
    ...
    num_params = sum([a.numel() for a in refernet.parameters()])
    print(f"Refernet params: {num_params / 1000 / 1000} M")
    params_to_clip = lora_params + list(refernet.parameters())
    ...
    union_model = UnionModel(transformer, refernet)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(union_model):
                ...
                reference_image = FluxPipeline._pack_latents(
                    model_input,
                    batch_size=model_input.shape[0],
                    num_channels_latents=model_input.shape[1],
                    height=model_input.shape[2],
