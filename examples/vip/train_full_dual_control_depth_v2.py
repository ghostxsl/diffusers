...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--init_model",
        action="store_true",
        help="Whether to initialize model."
    )
    parser.add_argument(
        "--compile",
        action="store_true",
...
def main(args):
    ...
    if args.init_model:
        # alter time_text_embed
        transformer.register_to_config(guidance_embeds=False)
        from diffusers.models.embeddings import CombinedTimestepTextProjEmbeddings
        time_text_embed = CombinedTimestepTextProjEmbeddings(transformer.inner_dim, transformer.config.pooled_projection_dim)
        time_text_embed.timestep_embedder.load_state_dict(transformer.time_text_embed.timestep_embedder.state_dict())
        time_text_embed.text_embedder.load_state_dict(transformer.time_text_embed.text_embedder.state_dict())
        transformer.time_text_embed = time_text_embed

        transformer.register_to_config(condition_embeds=True)
        transformer.condition_embedder = nn.Linear(transformer.config.in_channels, transformer.inner_dim)

        transformer.register_to_config(reference_embeds=True)
        transformer.reference_embedder = nn.Linear(transformer.config.in_channels, transformer.inner_dim)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                encoder_hidden_states = batch["prompt_embeds"].to(dtype=weight_dtype)
                pooled_projections = batch["pooled_prompt_embeds"].to(dtype=weight_dtype)
                ...
                model_pred = transformer(
                    ...
                    txt_ids=txt_ids,
                    img_ids=img_ids,
                    condition_embeds=packed_control_input,
                    reference_embeds=packed_refer_input,
                    return_dict=False,
                )[0]
