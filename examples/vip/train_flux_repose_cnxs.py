...
def save_models(output_dir, model, save_lora=False):
    ...
    for k, v in model.transformer.state_dict().items():
        ...
        elif save_lora and ("attn.norm_q" in k or "attn.norm_k" in k):
            ip_adapter_state_dict[k] = v
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = FluxControlNetXSModel.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing controlnet weights from transformer")
        controlnet = FluxControlNetXSModel.from_transformer(
            transformer,
    ...
    train_lora = args.rank is not None
    if train_lora:
        # Load lora weights
    ...
    ipa_params = [p for p in transformer.parameters() if p.requires_grad]
    params_to_optimize = ipa_params + list(controlnet.parameters())
    ...
    union_model = UnionModel(transformer, controlnet)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        save_models(save_path, unwrap_model(union_model), train_lora)
    ...
    if accelerator.is_main_process:
        union_model = unwrap_model(union_model)
        save_models(args.output_dir, union_model, train_lora)
