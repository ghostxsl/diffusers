...
def main(args):
    ...
    # Load IP Adapter
    state_dict = None
    if args.image_variation_model_path is not None:
    ...
    if args.pose_encoder_model_path:
        ...
        logger.info("Initializing pose encoder weights")
    ...
    num_params = sum([a.numel() for a in train_params])
    print(f"trained params: {num_params / 1000 / 1000} M")
    num_params = sum([a.numel() for a in list(pose_enc.parameters())])
    print(f"Pose encoder params: {num_params / 1000 / 1000} M")

    params_to_clip = train_params + list(pose_enc.parameters())
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(union_model):
                ...
                # # concat reference
                # reference_pixel_values = batch["reference_pixel_values"].to(dtype=vae.dtype)
                # refer_input = vae.encode(reference_pixel_values).latent_dist.sample()
                # refer_input = (refer_input - vae.config.shift_factor) * vae.config.scaling_factor
                # refer_input = refer_input.to(dtype=weight_dtype)
                #
                # refer_input = FluxPipeline._pack_latents(
                #     refer_input,
                ...
                # )
                # refer_input = refer_input * batch["uncond"].to(dtype=weight_dtype)
                ...
                model_pred = union_model(
                    ...
                    # torch.cat(
                    #     [packed_noisy_model_input, refer_input], dim=2),
                    timestep=timesteps,
