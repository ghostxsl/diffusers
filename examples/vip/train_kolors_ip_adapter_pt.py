...
def main(args):
    ...
    unet._init_ip_adapter_plus(
        embed_dims=1024, state_dict=ip_adapter_state_dict)
            ...
            if accelerator.is_main_process:
                for model in models:
                    if isinstance(model, ControlNetModel):
                        model.save_pretrained(join(output_dir, "controlnet"))
                    else:
                        save_ip_adapter_state_dict(join(output_dir, "ip_adapter"), model.state_dict())
                        if args.use_ema:
                            save_ip_adapter_state_dict(join(output_dir, "ema_ip_adapter"), ema_unet.state_dict)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, controlnet):
                ...
                unet_added_conditions["image_embeds"] = [image_embeds.unsqueeze(1)]
    ...
    if accelerator.is_main_process:
        ...
        controlnet.save_pretrained(join(args.output_dir, "controlnet"))
        if args.use_ema:
            save_ip_adapter_state_dict(join(args.output_dir, "ema"), ema_unet.state_dict)
