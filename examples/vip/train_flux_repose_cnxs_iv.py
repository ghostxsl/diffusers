...
def train_RMSNorm_params(model):
    ...
    for name, param in model.named_parameters():
        if "attn.norm_q" in name or "attn.norm_k" in name:
...
def save_models(output_dir, model):
    ...
    # save controlnet
    model.controlnet.save_pretrained(join(output_dir, "controlnet"))
    ...
    for k, v in model.transformer.state_dict().items():
        if "time_text_embed" in k or "context_embedder" in k:
            iv_state_dict[k] = v
        elif "attn.norm_q" in k or "attn.norm_k" in k:
            iv_state_dict[k] = v
...
def main(args):
    ...
    if args.image_variation_model_path:
        lora_state_dict = FluxPipeline.lora_state_dict(
            join(args.image_variation_model_path, "pytorch_lora_weights.safetensors"))
    ...
    if args.image_variation_model_path:
        state_dict = torch.load(
            join(args.image_variation_model_path, "image_variation.bin"), map_location='cpu')
    ...
    if args.controlnet_model_name_or_path:
        ...
        flux_controlnet = FluxControlNetXSModel.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing controlnet weights from transformer")
        flux_controlnet = FluxControlNetXSModel.from_transformer(
            transformer,
    ...
    text_ids = torch.zeros(added_seq_len, 3)
    ...
    # Optimizer creation
    trained_params = [p for p in transformer.parameters() if p.requires_grad]
    params_to_optimize = trained_params + list(flux_controlnet.parameters())
    ...
    union_model = UnionModel(transformer, flux_controlnet)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(union_model):
                ...
                if alter_x_embedder:
                    # Convert images to latent space
                    refer_input = vae.encode(
                        batch["reference_pixel_values"].to(dtype=vae.dtype)).latent_dist.sample()
                    refer_input = (refer_input - vae.config.shift_factor) * vae.config.scaling_factor
                    refer_input = refer_input.to(dtype=weight_dtype)
                    refer_input = refer_input * batch["uncond"].unsqueeze(-1).to(dtype=weight_dtype)
                    base_noisy_model_input = torch.cat([noisy_model_input, refer_input], dim=1)
                    base_noisy_model_input = FluxControlNetXSModel._pack_latents(base_noisy_model_input)

                noisy_model_input = FluxControlNetXSModel._pack_latents(noisy_model_input)
                ...
                model_pred = union_model(
                    ...
                    encoder_hidden_states=image_embeds.detach(),
                    pooled_projections=pooled_image_embeds.detach(),
                    img_ids=latent_image_ids.detach(),
                    txt_ids=text_ids.detach(),
                    guidance=guidance_vec.detach() if use_guidance_embeds else None,
                    base_hidden_states=base_noisy_model_input.detach() if alter_x_embedder else None,
                    return_dict=False,
                )[0]

                model_pred = FluxControlNetXSModel._unpack_latents(
                    model_pred, model_input.shape[2], model_input.shape[3])
            ...
            if accelerator.sync_gradients:
                ...
                # accelerator.distributed_type == DistributedType.DEEPSPEED or DeepSpeed requires saving weights on every device; saving weights only on the main process would cause issues.
                if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        ...
        flux_controlnet.save_pretrained(join(args.output_dir, "flux_controlnet"))
