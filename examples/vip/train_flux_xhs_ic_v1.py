...
from diffusers.data import FluxFillICT2IDataset, flux_fill_collate_fn
...
def main(args):
    ...
    txt_ids = torch.zeros(args.max_sequence_length, 3)
    txt_ids = txt_ids.to(device=accelerator.device, dtype=weight_dtype)
    ...
    transformer_lora_config = LoraConfig(
        ...
        target_modules=[
            "x_embedder", "context_embedder", "timestep_embedder.linear_1", "timestep_embedder.linear_2",
    ...
    train_dataset = FluxFillICT2IDataset(
        dataset_file=args.dataset_file,
        tokenizer_clip=tokenizer,
        tokenizer_t5=tokenizer_2,
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        collate_fn=flux_fill_collate_fn,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                refer_input = encode_vae_image(batch["reference_pixel_values"], vae)
                model_input = torch.cat([model_input, refer_input], dim=-1)
                ...
                masked_image = torch.cat([torch.zeros_like(refer_input), refer_input], dim=-1)
                mask = batch["mask"]

                noisy_model_input = torch.cat([noisy_model_input, masked_image, mask], dim=1)
                ...
                model_pred = transformer(
                    ...
                    img_ids=batch["latent_image_ids"].to(dtype=weight_dtype),
