...
class PTRandomCrop(object):
    def __init__(self,
        ...
        self.prob = prob
        self.scale_factor = scale_factor
        self.square_tag = isinstance(size, int)

    def scale_image(self, img, pose_img):
        w, h = img.size
        w = int(w // self.scale_factor * self.scale_factor)
        h = int(h // self.scale_factor * self.scale_factor)
        img = np.array(img)[:h, :w]
        pose_img = np.array(pose_img)[:h, :w]
        return Image.fromarray(img), Image.fromarray(pose_img)

    def pad_image(self, img, pose_img):
        w, h = img.size
        ...
        pose_img = np.pad(np.array(pose_img), pad_border, constant_values=0)

        return Image.fromarray(img), Image.fromarray(pose_img)

    def square_image_proc(self, img, pose_img):
        img, pose_img = self.pad_image(img, pose_img)
        img = img.resize((self.size, self.size), 1)
        pose_img = pose_img.resize((self.size, self.size), 0)
        return img, pose_img

    def center_crop(self, img, pose_img):
        _, image_height, image_width = F.get_dimensions(img)
        crop_height, crop_width = self.size
        ...
        if crop_width > image_width or crop_height > image_height:
            ...
            _, image_height, image_width = F.get_dimensions(img)
            if crop_width == image_width and crop_height == image_height:
                pose_img = F.pad(pose_img, padding_ltrb, fill=0)
                return img, pose_img

        crop_top = int(round((image_height - crop_height) / 2.0))
        crop_left = int(round((image_width - crop_width) / 2.0))
        img = F.crop(img, crop_top, crop_left, crop_height, crop_width)
        pose_img = F.crop(pose_img, crop_top, crop_left, crop_height, crop_width)
        return img, pose_img

    def other_image_proc(self, img, pose_img):
        w, h = img.size
        oh = self.size[0]
        ow = int(oh / h * w) if oh != h else w
        img = img.resize((ow, oh), 1)
        pose_img = pose_img.resize((ow, oh), 0)

        img, pose_img = self.center_crop(img, pose_img)

        return img, pose_img

    def get_data(self, data):
        img, pose_img = data['image'], data['condition_image']
        if self.square_tag:
            img, pose_img = self.square_image_proc(img, pose_img)
        else:
            img, pose_img = self.other_image_proc(img, pose_img)

        img, pose_img = self.scale_image(img, pose_img)

        data['image'] = img
        data['condition_image'] = pose_img
        return data
