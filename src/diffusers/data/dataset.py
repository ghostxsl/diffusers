...
class QwenImageDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.resize_cond = Resize((768, 768), interpolation=tv_transforms.InterpolationMode.LANCZOS)
        self.normalize_ = tv_transforms.Compose([
            ToTensor(),
            Normalize([0.5], [0.5], inplace=True),
        ])
    ...
    def __getitem__(self, index):
        ...
        if self.use_vae_latents:
            ...
            cond_image = load_image(item['product_img'])
            example["cond_image_latents"] = self.normalize_(self.resize_cond(cond_image))
            image = load_image(item['poster_img'])
