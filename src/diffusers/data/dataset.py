...
__all__ = [
    ...
    'FluxT2IDataset', 'FluxPTDataset', 'FluxRePoseDataset',
]
...
class IPAPoseTransDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.reference_transforms = tv_transforms.Compose(
            [
                HumanCrop(prob=1 - prob_uncond, random_crop=True),
                RandomHorizontalFlip(),
            ]
        )
...
class KolorsIPAPoseTransDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.reference_transforms = tv_transforms.Compose(
            [
                HumanCrop(prob=0.8, random_crop=True),
                RandomHorizontalFlip(),
            ]
        )
        self.normalize_ = Normalize([0.5], [0.5], inplace=True)
...
class FluxPTDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.image_transforms = tv_transforms.Compose([
            DrawPose(img_size, random_ratio=(0.5, 1.0)),
            RandomSelects([
                tv_transforms.Compose([
                    RandomResize(
                        img_size,
        ...
        self.reference_transforms = tv_transforms.Compose(
            [
                HumanCrop(prob=0.4, random_crop=True),
                Resize(img_size, interpolation=tv_transforms.InterpolationMode.LANCZOS),
            ]
        )
    ...
    def __getitem__(self, index):
        ...
        # prepare ip adapter input
        reference_data = self.reference_transforms(reference_data)
        example["reference_pixel_values"] = self.normalize_(reference_data["reference_image"])
...
class FluxRePoseDataset(torch.utils.data.Dataset):
    def __init__(
            self,
            dataset_file,
            clip_processor,
        ...
        self.image_transforms = tv_transforms.Compose([
            DrawPose(img_size, random_ratio=(0.625, 1.0)),
            ResizePad(img_size),
            ToTensor(),
            Normalize([0.5], [0.5], inplace=True),
        ])
