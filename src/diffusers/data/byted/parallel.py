...
def execute_concurrently(
            ...
            for future in completed:
                ...
                try:
                    ...
                    if retry_count[index] < max_retries:
                        ...
                        logger.warning(f"[execute_concurrently] retrying task at index {index} (retry {retry_count[index]}) due to exception: {e}")
                    else:
                        logger.error(f"[execute_concurrently] task at index {index} failed after {max_retries} retries, args: {args}, exception: {e}")
                        failed_tasks.append((index, e))

                        if fail_fast:
                            raise RuntimeError(
                                f"[execute_concurrently] task at index {index} failed after {max_retries} retries, args: {args}"
                            ) from e
    ...
    if failure_threshold is not None:
        ...
        if failure_rate > failure_threshold:
            raise RuntimeError(
                f"[execute_concurrently] failure rate {failure_rate:.2%} exceeds threshold {failure_threshold:.2%}, failed tasks: {len(failed_tasks)}/{len(args_list)}"
            )
