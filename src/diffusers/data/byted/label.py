...
GPT41Cli = MLLMClient(model_name="gpt-4.1-2025-04-14")
...
def label_batch_image(
    image_infos: List[ImageInfo], product_info: Dict = {}, label_mode: int = 0, use_cache: bool = True, cache_ttl: float = 0.5, web_url: str = ""
) -> List[ImageInfo]:
    """
    ...
    try:
        ...
        for image_info, res in zip(labeled_image_infos, results):
            if res:
                if image_info.Extra is None:
                    image_info.Extra = {}
                image_info.Extra["has_product"] = str(res.get("has_product", False)).lower()
                image_info.Extra["product_fully_visible"] = str(res.get("product_fully_visible", False)).lower()
                image_info.Extra["product_quantity"] = res.get("product_quantity", "")
                image_info.Extra["has_product_truncation"] = str(res.get("has_product_truncation", False)).lower()
                image_info.Extra["complex_surface"] = str(res.get("complex_surface", False)).lower()
                ...
                image_info.Extra["has_human"] = str(res.get("has_human", False)).lower()
                image_info.Extra["has_human_truncation"] = str(res.get("has_human_truncation", False)).lower()
                image_info.Extra["human_body_type"] = res.get("human_body_type", "")

                image_info.Extra["is_poster"] = str(res.get("is_poster", False)).lower()
                image_info.Extra["text_overlaps_product"] = str(res.get("text_overlaps_product", False)).lower()
                image_info.Extra["image_clarity"] = res.get("image_clarity", "")

                image_info.Extra["is_clothing_product"] = str(res.get("is_clothing_product", False)).lower()
                ...
                image_info.Extra["is_single_sided_view"] = str(res.get("is_single_sided_view", False)).lower()
                image_info.Extra["viewing_angle"] = res.get("viewing_angle", "")
                image_info.Extra["placement_naturalness"] = res.get("placement_naturalness", "")

                image_info.Extra["is_suitable_for_matting"] = str(res.get("is_suitable_for_matting", False)).lower()
