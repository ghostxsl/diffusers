...
def seedream_5_generate_image(
    ...
    assert ratio in size_map_2k, f"ratio must be one of {list(size_map_2k.keys())}"
    ...
    if image_urls:
        ...
        if isinstance(image_pils, Image.Image):
            data_body["image"] = f"data:image/jpeg;base64,{encode_pil_bytes(image_pils)}"
        else:
            data_body["image"] = []
            for image in image_pils:
                data_body["image"].append(f"data:image/jpeg;base64,{encode_pil_bytes(image)}")
    else:
        raise Exception(f"Parameters `image_urls` and `image_pils` are both empty")
    ...
    if response_format == "url":
        ...
        return base64_to_img(imagesResponse.data[0].b64_json)
