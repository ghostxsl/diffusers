...
class FluxImageVariationPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def __init__(
        ...
        self.structural_control = structural_control
    ...
    def prepare_latents(
        ...
        if latents is not None:
            latent_image_ids = self._prepare_latent_image_ids(
                batch_size, latents.shape[2], latents.shape[3], device, dtype)
        ...
        latent_image_ids = self._prepare_latent_image_ids(
            batch_size, image_latents.shape[2], image_latents.shape[3], device, dtype)
    ...
    def encode_image_embeds(
        ...
        if not use_image_encoder:
            ...
            negative_pooled_image_embeds = None
    ...
    def __call__(
        ...
        **kwargs
        ...
        use_image_encoder = self.image_encoder is not None
        ...
        if self.alter_x_embedder:
            ref_image = self.image_processor.preprocess(reference_image, height=height, width=width)
            ref_image = ref_image.to(device=device, dtype=image_embeds.dtype)
            ref_latents = self._encode_vae_image(image=ref_image, generator=generator)
            ref_latents = self._pack_latents(
                ref_latents, num_images_per_prompt,
                ref_latents.shape[1],
                ref_latents.shape[2],
                ref_latents.shape[3])
        ...
        if control_image is not None:
            ...
            control_image = control_image.to(device, dtype=image_embeds.dtype)

            if self.structural_control:
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.controlnet is not None:
                    ...
                    noise_pred = self.controlnet(
                        ...
                        base_hidden_states=None if not self.alter_x_embedder else \
                            torch.cat([latents, ref_latents], dim=-1),
                        return_dict=False,
                    )[0]
                elif self.structural_control:
                ...
                if do_classifier_free_guidance:
                    if self.controlnet is not None:
                        ...
                        noise_pred_uncond = self.controlnet(
                            base_model=self.transformer,
                            hidden_states=latents,
                            controlnet_cond=control_image,
                            conditioning_scale=cond_scale,
                            ...
                            joint_attention_kwargs=self.joint_attention_kwargs,
                            base_hidden_states=None if not self.alter_x_embedder else \
                                torch.cat([latents, negative_ref_latents], dim=-1),
                            return_dict=False,
                        )[0]
                    elif self.structural_control:
