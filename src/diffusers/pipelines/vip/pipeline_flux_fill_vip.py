    ...
    def __call__(
        ...
        **kwargs
        ...
        for i in range(num_images[0]):
            ...
            for j in range(num_images[1]):
                if i == 0 and j == 0:
                    row_image_latents.append(image_latents)
                else:
                    row_image_latents.append(zero_latents)
            row_image_latents = torch.cat(row_image_latents, dim=-1)
            masked_image_latents.append(row_image_latents)
        masked_image_latents = torch.cat(masked_image_latents, dim=-2)

        masked_image_latents = self._pack_latents(
            masked_image_latents,
            masked_image_latents.shape[0],
            masked_image_latents.shape[1],
            masked_image_latents.shape[2],
            masked_image_latents.shape[3],
        )

        mask_zeros = torch.zeros([num_images_per_prompt, height, width])
        mask_ones = torch.ones([num_images_per_prompt, height, width])
        mask = []
        for i in range(num_images[0]):
            row_mask = []
            for j in range(num_images[1]):
        ...
        mask = mask.view(
            num_images_per_prompt,
            h * num_images[0], self.vae_scale_factor,
            w * num_images[1], self.vae_scale_factor,
        )  # batch_size, height, 8, width, 8
        mask = mask.permute(0, 2, 4, 1, 3)  # batch_size, 8, 8, height, width
        mask = mask.reshape(
            num_images_per_prompt, self.vae_scale_factor * self.vae_scale_factor,
            h * num_images[0], w * num_images[1]
        )  # batch_size, 8*8, height, width
        ...
        masked_image_latents = torch.cat((masked_image_latents, mask), dim=-1).to(device=device, dtype=prompt_embeds.dtype)

        # 6. Prepare timesteps
        sigmas = np.linspace(1.0, 1 / num_inference_steps, num_inference_steps)
        image_seq_len = latents.shape[1] // 2
        mu = calculate_shift(
            image_seq_len,
