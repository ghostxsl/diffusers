...
class FluxFillPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        if isinstance(generator, list):
            image_latents = [
                retrieve_latents(self.vae.encode(image[i : i + 1].to(self.vae.dtype)), generator=generator[i])
                for i in range(image.shape[0])
            ]
            image_latents = torch.cat(image_latents, dim=0)
        else:
            image_latents = retrieve_latents(self.vae.encode(image.to(self.vae.dtype)), generator=generator)
    ...
    def prepare_latents(
        ...
        if image_latents is not None:
            ...
            velocity = noise - image_latents
            velocity = self._pack_latents(velocity, *velocity.shape)
        else:
            latents = noise
            velocity = None

        latents = self._pack_latents(latents, *latents.shape)

        return latents, latent_image_ids, velocity
    ...
    def __call__(
        ...
        guidance_vector: float = 30.0,
        num_images_per_prompt: Optional[int] = 1,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        joint_attention_kwargs: Optional[Dict[str, Any]] = None,
        max_sequence_length: int = 512,
        latent_guidance_step: int = 0,
        v_scale: float = 0.001,
        **kwargs
    ):
        ...
        # 4.1 prepare masked_image and image_latents
        image_latents = self._encode_vae_image(image=init_image, generator=generator)
        masked_image = torch.cat([torch.zeros_like(image_latents), image_latents], dim=-1)
        ...
        if tgt_image is not None:
            ...
            tgt_image_latents = self._encode_vae_image(image=tgt_image, generator=generator)
            image_latents = torch.cat([tgt_image_latents, image_latents], dim=-1)

        # 4.2 prepare fill mask
        vae_scale_factor = 8
        latent_h = int(height / vae_scale_factor)
        latent_w = int(width / vae_scale_factor) * 2

        mask = torch.cat([
            torch.ones([num_images_per_prompt, height, width]),
            torch.zeros([num_images_per_prompt, height, width]),], dim=-1)
        mask = mask.view(
            num_images_per_prompt, latent_h, vae_scale_factor, latent_w, vae_scale_factor
        )  # batch_size, height, 8, width, 8
        mask = mask.permute(0, 2, 4, 1, 3)  # batch_size, 8, 8, height, width
        mask = mask.reshape(
            num_images_per_prompt, vae_scale_factor * vae_scale_factor, latent_h, latent_w
        ...
        latents, latent_image_ids, velocity = self.prepare_latents(
        ...
        condition_latents = torch.cat([masked_image, packed_mask], dim=-1).to(dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                res = self.scheduler.step(
                    ...
                    latent_guidance=i < latent_guidance_step,
                    velocity=velocity,
                    v_scale=v_scale,
                    return_x0=kwargs.get("return_gif", False),
                    return_dict=False
                )
                latents = res[0]
                if kwargs.get("return_gif", False):
                    x0 = res[1].to(self.vae.dtype)
                    x0 = self._unpack_latents(x0, height, img_width, self.vae_scale_factor)
                    x0 = (x0 / self.vae.config.scaling_factor) + self.vae.config.shift_factor
                    x0_image = self.vae.decode(x0, return_dict=False)[0]
                    x0_image = self.image_processor.postprocess(x0_image, output_type="pil")[0]
                    pred_x0_list.append(x0_image)
