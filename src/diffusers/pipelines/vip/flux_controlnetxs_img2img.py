...
class FluxControlnetXSImg2ImgPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        tokenizer_2: T5TokenizerFast,
        transformer: FluxTransformer2DModel,
        controlnet: FluxControlNetXSModel,
    ...
    def prepare_ip_adapter_image_embeds(
            self, ip_adapter_image, device, num_images_per_prompt, do_classifier_free_guidance=False
    ):
        if ip_adapter_image is None:
            ip_adapter_image = Image.new("RGB", (336, 336))

        image_embeds, negative_image_embeds = self.encode_image(
            ip_adapter_image, device, 1, True)
        image_embeds = torch.cat([image_embeds] * num_images_per_prompt, dim=0)
        negative_image_embeds = torch.cat(
            [negative_image_embeds] * num_images_per_prompt, dim=0
        )
