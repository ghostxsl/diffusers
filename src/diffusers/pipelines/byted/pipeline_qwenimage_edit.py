...
def calculate_dimensions(target_area, ratio, mod_value=16):
...
class QwenImageEditPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def __call__(
        ...
        prompt_image_size: tuple[int, int] = (224, 224),
        reference_image_size: tuple[int, int] = (768, 768),
    ):
        r"""
        ...
        prompt_image_size = calculate_dimensions(
            prompt_image_size[0] * prompt_image_size[1],
            image_reference.width / image_reference.height,
            mod_value=14,
        )
        prompt_image = image_reference.resize(prompt_image_size , 1)
        ...
        ref_w, ref_h = calculate_dimensions(
            reference_image_size[0] * reference_image_size[1],
            image_reference.width / image_reference.height
        )
        ref_image = self.image_processor.preprocess(image_reference, ref_w, ref_h)
        ...
        if mask_image is not None:
            ...
            image = self.image_processor.preprocess(image, width, height)
