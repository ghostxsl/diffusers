...
from ...utils import BaseOutput, deprecate, logging, scale_lora_layers, unscale_lora_layers
...
class SimReferenceNetModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        refer_out_channels: Tuple[int] = (320, 640, 1280, 1280),
                ...
                for attn_module in down_block.attentions:
                    if hasattr(attn_module, 'transformer_blocks'):
                        attn_module.transformer_blocks = nn.ModuleList(
                            [SimAttnIdentity(
                                block_out_channels[i], refer_out_channels[i]
                            ) for _ in attn_module.transformer_blocks])
        ...
        if self.mid_block is not None and hasattr(self.mid_block, 'attentions'):
            for attn_module in self.mid_block.attentions:
                    attn_module.transformer_blocks = nn.ModuleList(
                        [SimAttnIdentity(
                            block_out_channels[-1], refer_out_channels[-1]
                        ) for _ in attn_module.transformer_blocks])
        ...
        reversed_refer_out_channels = list(reversed(refer_out_channels))
            ...
            if hasattr(up_block, 'attentions'):
                for attn_module in up_block.attentions:
                    if hasattr(attn_module, 'transformer_blocks'):
                        attn_module.transformer_blocks = nn.ModuleList(
                            [SimAttnIdentity(
                                reversed_block_out_channels[i], reversed_refer_out_channels[i]
                            ) for _ in attn_module.transformer_blocks])
...
class SimAttnIdentity(nn.Module):
    def __init__(self, in_channels, out_channels, **kwargs: Any) -> None:
        super().__init__()
