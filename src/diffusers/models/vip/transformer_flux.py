...
from ...models.embeddings import IPAdapterPlusImageProjection
...
class IPAdapterFluxAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        # IP-Adapter
        if image_embeds is not None and txt_seq_len is not None and self.scale > 0:
            ...
            ip_encoder_hidden_states, hidden_states = (
                hidden_states[:, :txt_seq_len],
                hidden_states[:, txt_seq_len:],
            )
            hidden_states = hidden_states + self.scale * current_ip_hidden_states
            hidden_states = torch.cat([ip_encoder_hidden_states, hidden_states], dim=1)
...
class FluxSingleTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if image_embeds is not None and txt_seq_len is not None:
...
class FluxTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def _init_ip_adapter_plus(self,
        ...
        for name, module in self.attn_processors.items():
            attn_procs[name] = IPAdapterFluxAttnProcessor2_0(
                embed_dims=embed_dims, hidden_dims=self.inner_dim, scale=scale,
    ...
    def forward(
        ...
        txt_seq_len = None
        if image_embeds is not None and hasattr(self, "ip_image_proj"):
