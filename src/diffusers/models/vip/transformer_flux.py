...
class FluxTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def _init_image_variation(self,
                              ...
                              state_dict=None,
                              alter_x_embedder=False):
        self.register_to_config(guidance_embeds=use_guidance_embeds)
        text_time_guidance_cls = (
        ...
        if self.config.guidance_embeds:
            time_text_embed.timestep_embedder.requires_grad_(False)

            time_text_embed.guidance_embedder.load_state_dict(self.time_text_embed.guidance_embedder.state_dict())
            time_text_embed.guidance_embedder.requires_grad_(False)
    ...
    def _init_concat_x_embedder(self, state_dict=None):
        # concat reference latents
        x_embedder_weights = self.x_embedder.weight.data
        x_embedder_bias = self.x_embedder.bias.data
    ...
    def forward(
        ...
        **kwargs
        ...
        if controlnet_cond is not None:
            hidden_states = hidden_states + controlnet_cond * conditioning_scale
