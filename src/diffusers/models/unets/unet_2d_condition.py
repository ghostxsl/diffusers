...
class UNet2DConditionModel(
    ...
    def _set_add_embedding(
        ...
        elif self.config.addition_embed_type == "vip_image":
            self.add_embedding = TimestepEmbedding(projection_class_embeddings_input_dim, time_embed_dim)
    ...
    def get_aug_embed(
        ...
        if self.config.addition_embed_type == "text":
            ...
            if "pooled_image_embeds" not in added_cond_kwargs:
                raise ValueError(
                    f"{self.__class__} has the config param `addition_embed_type` set to 'vip_image' which requires the keyword argument `pooled_image_embeds` to be passed in `added_cond_kwargs`"
                )
            pooled_image_embeds = added_cond_kwargs.get("pooled_image_embeds")
            aug_emb = self.add_embedding(pooled_image_embeds)
