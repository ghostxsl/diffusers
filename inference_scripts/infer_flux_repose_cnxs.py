...
from diffusers.pipelines.vip.flux_controlnetxs_img2img import FluxControlnetXSImg2ImgPipeline
...
def main(args):
    ...
    transformer._init_ip_adapter_plus(
        ...
        state_dict=torch.load(
            join(args.pt_model_path, "ip_adapter_plus.bin"),
    ...
    controlnet = FluxControlNetXSModel.from_pretrained(join(args.pt_model_path, "controlnet")).to(device, dtype=dtype)

    pipe = FluxControlnetXSImg2ImgPipeline(
        scheduler=scheduler,
        vae=vae,
