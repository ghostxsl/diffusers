...
def parse_args():
    ...
    parser.add_argument(
        "--flux_concat_model_path",
        type=str,
        default="/apps/dat/cv/xsl/exp_animate/flux_concat",
    )
    parser.add_argument(
        "--alter_x_embedder",
        action="store_true",
...
def main(args):
    ...
    transformer._init_concat_x_embedder(
        state_dict=torch.load(join(args.flux_concat_model_path, "x_embedder.bin"), map_location=torch.device("cpu"))
        )
    ...
    pipe.load_lora_weights(join(args.flux_concat_model_path, "pytorch_lora_weights.safetensors"))
