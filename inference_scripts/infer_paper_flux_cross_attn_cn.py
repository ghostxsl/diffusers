...
def main(args):
    ...
    controlnet = None
    controlnetxs = None
    if args.controlnetxs_model_path:
        controlnetxs = FluxControlNetXSModel.from_pretrained(args.controlnetxs_model_path).to(device, dtype=dtype)
    else:
        controlnet = FluxControlNetModel.from_pretrained(args.controlnet_model_path).to(device, dtype=dtype)
