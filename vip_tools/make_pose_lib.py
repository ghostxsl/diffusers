...
def main(args):
    ...
    for name in tqdm(pose_list):
        try:
            ...
            out_pose["body"]["dts"].append(body_kpts)
            out_pose["hand"]["dts"].append(hand_kpts)
            out_pose["face"]["dts"].append(face_kpts)
            out_pose["file_name"]["dts"].append(splitext(name)[0] + '.jpg')
