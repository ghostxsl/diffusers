...
def normalize_pose(pose, crop_size=(1024, 768)):
    ...
    # 取关键点的外接矩和预测的bbox的并集
    bbox_kpts = body_kpts[body_kpts[:, -1] > 0.3, :2]
    min_xy = np.min(bbox_kpts, axis=0)
    max_xy = np.max(bbox_kpts, axis=0)
    bbox[:2] = np.min(np.stack([bbox[:2], min_xy]), axis=0)
    bbox[2:] = np.max(np.stack([bbox[2:], max_xy]), axis=0)
    # 平移关键点
    body_kpts[..., :2] -= bbox[:2]
    hand_kpts[..., :2] -= bbox[:2]
    face_kpts[..., :2] -= bbox[:2]
