...
class PoseTransfer(object):
    def __init__(self, args, crop_size=(768, 576), draw_hand=True, ip_adapter_scale=0.9):
        ...
        self.restore_hand = args.restore_hand
        ...
        self.pipe_pose_trans = VIPStableDiffusionControlNetInpaintPipeline(
            vae=vae,
        ...
        if self.restore_hand:
            self.parsing_infer = HumanParsing(
                model_path=join(self.weight_dir, "extensions/deeplabv3plus-xception-vocNov14_20-51-38_epoch-89.pth"),
                infer_size=640,
                device=self.device,
            )
    ...
    def restore_hand_image(self, image, generator):
        w, h = image.size
        # get hand mask
        label_parsing = self.parsing_infer(image)
        hand_mask = np.zeros_like(label_parsing)
        hand_mask[label_parsing == 14] = 255
        hand_mask[label_parsing == 15] = 255
        hand_mask = dilate_mask(hand_mask, dilate_factor=2)
        # get crop bbox
        hand_bbox = get_bbox_from_mask(hand_mask)
        if hand_bbox is None:
            return image
        crop_region = pad_bbox_to_square(hand_bbox, [h, w])
        crop_region = pad_bbox(crop_region, [h, w], padding=32)

        hand_mask = mask_process(Image.fromarray(hand_mask), invert_mask=False)
        # get pose
        pose_img, pose = self.pose_infer(image)
        pose_img = Image.fromarray(pose_img)

        # self.pipe_pose_trans.set_ip_adapter_scale(0.0)

        crop_img = image.crop(crop_region)
        mask_image = hand_mask.crop(crop_region)
        crop_pose_img = pose_img.crop(crop_region)

        res_img, image_overlay = self.pipe_pose_trans(
            prompt=f"good hand, high-res, best quality",
            negative_prompt="(worst quality:2), (low quality:2), lowres",
            image=crop_img,
            mask_image=mask_image,
            control_image=crop_pose_img,
            ip_adapter_image=crop_img,
            height=768,
            width=768,
            strength=0.6,
        ...
        res_img = alpha_composite(res_img, image_overlay)[0]
        x1, y1, x2, y2 = crop_region
        res_image = image.copy()
        res_image.paste(res_img.resize((int(x2 - x1), int(y2 - y1)), resample=Image.LANCZOS), (x1, y1))

        # self.pipe_pose_trans.set_ip_adapter_scale(self.ip_adapter_scale)
        return res_image
    ...
    def topk_pose(self, pose, topk=1, pose_label="dts"):
        ...
        # norm_pose_img = [self.draw_pose(norm_pose, self.crop_size)]
        ...
        for ind, pose_similarity in zip(topk_ind, topk_oks):
            ...
            # norm_pose_img.append(
            #     self.draw_pose(body_pose, self.crop_size,
    ...
    def __call__(self, image, topk=1, pose_label="dts", **kwargs):
        ...
        mask_image = Image.new("RGB", self.infer_size[::-1], color=(255, 255, 255))
        # ip-adapter image
        ...
        for pose_img in topk_pose_img:
            ...
            res_img = self.codeformer(res_img)
            res_img = alpha_composite(res_img, image_overlay)[0]
            ...
            if self.restore_hand:
                res_img = self.restore_hand_image(res_img, generator)
                out_restore_img.append(res_img)

        out_pose.insert(0, src_pose_img)
        return out_img, out_pose, topk_info, out_restore_img
...
def main(args):
    ...
    for i, file in enumerate(img_list):
        try:
            ...
            out_imgs, out_pose, topk_info, out_restore_img = pose_trans(
            ...
            if args.display:
                ...
                if args.restore_hand:
                    out_restore_img = np.concatenate([crop_img, ] + out_restore_img, axis=1)
                    out_imgs = np.concatenate([out_imgs, out_restore_img, out_pose])
                else:
