    ...
    if args.rec:
        ...
        def rec_single_process(gen_path, gt_path):
            ...
            if args.split:
                gen_img = np.split(np.array(gen_img), 3, axis=1)[-1]
                gen_img = Image.fromarray(gen_img)
            gen_img = gen_img.resize(args.resolution[::-1], Image.LANCZOS)
            gen_img = np.array(gen_img).astype(np.float32)

            gt_img = Image.open(gt_path).resize(args.resolution[::-1], Image.LANCZOS)
        ...
        print(f"SSIM: {round(np.mean(ssim_result), 4)}")
        print(f"PSNR: {round(np.mean(psnr_result), 4)}")
        print(f"L1: {round(np.mean(l1_result), 4)}")
