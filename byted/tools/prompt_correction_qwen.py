...
prompt = """
Task Requirements:
1. The rewritten Prompt shall be based on the image, while the Prompt provided by the user shall serve as a reference—this is because errors may exist in the Prompt input by the user.
2. For overly brief user inputs, reasonably infer and add details to enhance the visual completeness without altering the core content;
3. Refine descriptions of subject characteristics, visual style, spatial relationships, and shot composition;
4. If the input requires rendering text in the image, enclose specific text in quotation marks, specify its position (e.g., top-left corner, bottom-right corner) and style. This text should remain unaltered and not translated. For the prompts entered by users, proofreading should mainly focus on the **content and position of the text**, and the response shall be based on the content in the image. For example: If a word appears in **all uppercase form** in the image, the word entered by the user should be rewritten into all uppercase form.
5. Match the Prompt to a precise, niche style aligned with the user’s intent. If unspecified, choose the most appropriate style (e.g., realistic photography style, fashion photography style, abstract expressionist style, minimalist style, romantic style, futurist style, conceptual art style, expressionist style);
6. Except for the text content explicitly required by the user, **no additional text content is allowed**.
7. Please ensure that the Rewritten Prompt is less than 300 words and does not contain line breaks.
...
3. Black-haired Chinese adult male, portrait above the collar. A black cat's head blocks half of the man's side profile, sharing equal composition. Shallow green jungle background. Graffiti style, clean minimalism, thick strokes.
...
info_dict = load_csv_or_xlsx_to_dict("/mnt/bn/creative-algo/xsl/results_all_done_4567_v3.xlsx")

for item in tqdm(info_dict):
    try:
        result_prompt = gpt_4_1_client.make_image_request(
            sys_prompt,
            prompt + item['gen_text_prompt'] + " " + item['gen_bg_prompt'],
...
xlsx_save(info_dict, "results_all_done_4567_v3.5.xlsx")
print('done')
