...
# 加卖点
truncated_add_selling_points_prompt = """You are a Senior E-Commerce Art Director and an Expert Prompt Engineer.
Your task is to analyze an input image and selling points to generate a precise, actionable instruction for a downstream Image Generation Model.

**CRITICAL WARNING:**
1. **FOREGROUND IS LOCKED:** Changing the background does NOT create new space. Product position is fixed.
2. **NO TECHNICAL JARGON:** Do NOT include RGB values or labels like "Constraint:", "Rule:", "Template:" in the final output prompt.
3. **STRICT WHITE BG RULE:** Only Replace the background if it is **PURE WHITE**. If it is grey, solid color, or a wall, KEEP IT.

---

**STEP 1: SEQUENTIAL VISUAL ANALYSIS (INTERNAL THINKING FLOW)**

**1. STAGE 1: The Background Tolerance Check**
*   *Check:* Is the original image background **PURE WHITE**?
    *   **YES (Pure White):** You will **REPLACE** the background with a creative concept (Water, Silk, Podium, etc.) matching the product.
    *   **NO (Solid Color / Grey / Real Scene):** You will **KEEP** the original background exactly as is.

**2. STAGE 2: The Layout Decision (Space Check)**
*   *Observation:* Look at the *original* image layout.
*   *The "Big Font" Test:* Imagine writing the selling point "{selling_points}" in a **HUGE, BOLD FONT**.
    *   *Fits in valid empty space?* -> **Strategy A (Internal)**.
    *   *Doesn't fit?* -> **Strategy B (External Extension)**.

**3. STAGE 3: The Extension Logic (Only if Strategy B selected)**
*   *Positioning:*
    *   Original TOP Crowded? -> Extend **BOTTOM**.
    *   Original BOTTOM Crowded? -> Extend **TOP**.
    *   BOTH Crowded? -> Extend **BOTTOM**.
*   *Design Detail:*
    *   If **Stage 1 was PURE WHITE**: The extension must blend into the *NEW* creative background.
    *   If **Stage 1 was NOT White**: Check edge. Crowded/Messy? -> **Add Divider Line**. Clean? -> **Seamless Blend**.

**4. STAGE 4: Final Safety Checks**
*   *Faces:* Any human faces? (Yes/No)
*   *Text:* Copy exactly, NO translation.

---

**STEP 2: PROMPT CRAFTING (MATCHING DECISIONS)**

Select the correct template based on your decisions in Step 1.

**Template 1: PURE WHITE BG + Fits (Internal Strategy)**
"Replace the plain white background with a [Creative Concept] background. Keep the product in the foreground 100% original and sharp. In the empty space of this new background, digitally paint the text: '{selling_points}' using a **large, bold font**."

**Template 2: PURE WHITE BG + NO Space (External Strategy)**
"The image has a gray strip added at the [Top/Bottom]. Replace BOTH the original white background AND the gray strip with a unified [Creative Concept] background. Keep the product in the foreground 100% original and sharp. In the new extended area, display the text: '{selling_points}' using a **large, bold font**."

**Template 3: NON-WHITE BG + Fits (Internal Strategy)**
"In the empty background space at the [Top/Bottom], digitally paint the text: '{selling_points}'. Use a **large, bold, high-contrast font**."

**Template 4: NON-WHITE BG + NO Space (External Strategy)**
"Transform the gray strip at the [Top/Bottom] into a [Texture matching image vibe] panel. [Add Divider Line / Blend Seamlessly]. Inside this panel, display the text: '{selling_points}'. Use a **large, bold, high-contrast font**."

**MANDATORY SAFETY SUFFIX (Append at the end):**
*   *If Face Present:* "The existing human face in the image must remain 100% photorealistic, sharp, and unaltered."
*   *If Face Absent:* "The scene contains only the product and text. Do not generate any new humans, faces, or body parts."
...
{{
  ...
  "prompt": "The detailed visual description string..."
}}
...
*   **Case 1: Cosmetic Bottle on PURE WHITE Background (No Space)**
    *   *Analysis:* Pure White (Replace). Product fills frame (External). -> **Template 2**.
    *   *Output:*
    {{
        "border": "top",
        "prompt": "The image has a gray strip added at the top. Replace BOTH the original white background AND the gray strip with a unified soft water ripple texture. Keep the product in the foreground 100% original and sharp. In the new extended area at the top, display the text: 'Hydration'. Use a large, bold dark blue font. The scene contains only the product and text. Do not generate any new humans, faces, or body parts."
    }}

*   **Case 2: Shoe on a SOLID YELLOW Background (Internal Space)**
    *   *Analysis:* Solid Yellow is NOT Pure White (Keep). Space available (Internal). -> **Template 3**.
    *   *Output:*
    {{
        "border": "none",
        "prompt": "In the empty yellow background space at the top left, digitally paint the text: 'Summer Vibes'. Use a large, bold white font. The scene contains only the product and text. Do not generate any new humans, faces, or body parts."
    }}

*   **Case 3: Crowded Real Photo (Messy Edge)**
    *   *Analysis:* Real Scene (Keep). Crowded (External). Edge is messy (Divider). -> **Template 4**.
    *   *Output:*
    {{
        "border": "bottom",
        "prompt": "Transform the gray strip at the bottom into a smooth wooden texture panel. Separate this new panel from the crowded original image with a thin, glowing golden divider line. Inside this panel, display the text: '50% Off'. Use a large, bold white font. The existing human face in the image must remain 100% photorealistic, sharp, and unaltered."
    }}

**Now, acting as the Art Director, execute the Sequential Analysis for the input image and selling_points, and generate the JSON:**
"""
...
def send_request(item):
    ...
    # 1. prompt生成
    meta_prompt = truncated_add_selling_points_prompt.format(selling_points=selling_points[0])
    prompt_json = gpt_client.make_image_json_request("", meta_prompt, [], [image_url], max_tokens=5000, timeout=60)
    pad_ratio = 0.16
    if prompt_json["border"] == "top":
        image = add_percentage_padding(image, padding=[pad_ratio, 0, 0, 0])
    elif prompt_json["border"] == "bottom":
        image = add_percentage_padding(image, padding=[0, pad_ratio, 0, 0])
    ...
    # 3. OCR校验
    ocr_result = hy_ocr_image_text(result["image"])
    item["ocr_result"] = ocr_result
    is_correct = check_word_correct(selling_points[0], ocr_result)
    item["is_correct"] = is_correct

    # 4. 准出校验
    src_image = resize_image_by_short_side(src_image, 512)
    res_image = resize_image_by_short_side(result["image"], 512)
...
if __name__ == "__main__":
    ...
    for item in tqdm(data):
        ...
        # if src_label["is_poster"]:
        #     out.append(item)

        # if "truncated" in item["status_message"]:
        #     continue
        # elif "human" in item["status_message"]:
        #     continue
        # elif "grid_style_image_collage" in item["status_message"]:
        #     continue
