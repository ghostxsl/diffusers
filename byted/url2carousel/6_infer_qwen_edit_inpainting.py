...
def get_qwen_inpainting_images(image, mask, position="center", ratio="9:16", margin=20):
    W, H = aspect_ratios[ratio]
    max_w, max_h = 0.7 * W, 0.5 * H
    resize_ratio = min(max_w / image.width, max_h / image.height)
    out_w, out_h = int(image.width * resize_ratio), int(image.height * resize_ratio)

    image = image.convert('RGB').resize((out_w, out_h), 1)
    ...
    rand_y = random.randint(0, int(H * 0.1))
...
def main(args):
    ...
    pipe = QwenImageEditPipeline.from_pretrained(args.pretrained_model_path, torch_dtype=dtype)
    pipe = pipe.to(device)
    ...
    for line in tqdm(data_list):
        ...
        product_mask_image = decode_pil_bytes(get_file_from_tos(line['product_mask_url']).data, False)
        image, mask_image = get_qwen_inpainting_images(
            product_img,
            product_mask_image,
