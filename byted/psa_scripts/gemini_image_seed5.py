...
from diffusers.data.byted.clients.azure_mllm import MLLMClient, seedream_5_generate_image, gemini_25_flash_image_gen
...
from diffusers.data.utils import load_file, json_save, resize_image_by_short_side


image_gen_model = "seed-5"
...
def send_request(item):
    ...
    if image_gen_model == "gemini-2.5":
        ...
        gen_url1 = save_tos(encode_pil_bytes(img1, False), _gen_name(""), headers={"Content-Type": "image/jpeg"})
    ...
    # res_img = seedream_5_generate_image(
    #     prompt_91,
    #     image_pils=resize_image_by_short_side(src_image, 1024),
    #     specify_gen_ratio=True,
    #     ratio=gen_ratio,
    #     image_size="2K",
    #     response_format="b64_json",
    # )
    # img2 = res_img.resize((res_img.width // 2, res_img.height // 2), 1)
    # gen_url2 = save_tos(encode_pil_bytes(img2, False), _gen_name(""), headers={"Content-Type": "image/jpeg"})

    item.append([gen_url1, ])
    return item
