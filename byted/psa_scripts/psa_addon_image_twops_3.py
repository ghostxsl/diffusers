...
from diffusers.data.byted.clients.ad_creative_ocr_quality_control import validate_ocr_quality_control_v2
...
gpt_client = MLLMClient(model_name="gpt-5-mini-2025-08-07", api_key="lV9PRXdcOPUV8AgbdrTUtf8E9B0r68Qc_GPT_AK")
gemini_client = MLLMClient(model_name="gemini-3.1-fl", api_key="XpGAiljn5ZhzjpRn8Eo8BRvj1uWcaCmP_GPT_AK")
...
def send_request(item):
    ...
    # 1. prompt生成
    meta_prompt = poster_pip_add_text_3_4.format(first_selling_points=selling_points[0], second_selling_points=selling_points[1])
    prompt_bg = gemini_client.make_image_request("", meta_prompt, [], [image_url], image_pils=[], max_tokens=5000, timeout=60)
    ...
    # 3. OCR校验
    is_correct, add_tokens, ocr_result = validate_ocr_quality_control_v2(gen_image_url=gen_url1, src_image_url=image_url, check_text_list=selling_points[:2])
    item["is_correct"] = is_correct
    item["ocr_add_tokens"] = add_tokens
