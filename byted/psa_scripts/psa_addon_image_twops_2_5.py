...
1.  **Input State:** The input is a pre-expanded 3:4 canvas, with the original product photo (any aspect ratio) centered, and the top/bottom areas pre-filled with flat grey.
2.  **Central Image Preservation (FINAL CONCLUSION ONLY):**
    - If the **entire central image background is a single solid color** (e.g., pure white, pure black, or a uniform single color across the whole background): Redesign the central image’s background with a style-matching texture/pattern/gradient (preserve all foreground elements 100% intact).
    - If the central image has a **mixed background** (e.g., half white and half black, textured, or a real scene): Output ONLY the final instruction: **"Keep the central product photo completely unchanged (no edits, filtering, cropping, or pixel changes)"**.
    - Do NOT include any reasoning, background analysis, or explanation in the output prompt.
3.  **Fixed Face Protection Mandate (MUST INCLUDE IN OUTPUT):** Every output prompt must explicitly contain the exact phrase: **"Keep the face in the original image clear and do NOT generate any new faces"**.
4.  **Dual Selling Points with Hierarchy:**
    - **Primary Selling Point (`{first_selling_points}`):** Place as a single continuous line in the **TOP GREY AREA** (larger space). Use a **LARGER, BOLD font** to highlight it. Wrap text in double quotes (e.g., "Tanpa alkohol & minyak").
    - **Secondary Selling Point (`{second_selling_points}`):** Place as a single continuous line in the **BOTTOM GREY AREA** (smaller space). Use a **SMALLER font** (one size down from primary) for a secondary visual weight. Wrap text in double quotes (e.g., "Dengan Vitamin C").
    - Both texts must not overlap with the central image, have no line breaks/truncation, and their font/color must match the overall design tone.
5.  **Prohibited Technical Language:** Do NOT use any procedural/technical terms (e.g., RGB values, color codes, pixel coordinates). Describe all visual attributes using natural language (e.g., "flat grey", "warm orange", "soft radial gradient").
6.  **Forbidden Content:** Do NOT include any internal strategy labels (e.g., "Strategy 1:"), reasoning, or background analysis in the output prompt. Only output clear, actionable instructions.

===== CHAIN OF THOUGHT (INTERNAL ONLY, DO NOT OUTPUT) =====
**Step 1: Analyze Central Image Background**
- Check if the **entire central image background is a single solid color** → if yes, redesign its background; if no (mixed/textured/real scene), keep it 100% intact.
**Step 2: Design Top/Bottom Grey Areas**
- Create a cohesive background texture/pattern for the top/bottom grey areas that matches the product and selling points.
- Place primary/secondary selling points with hierarchical font sizes and style-matching colors.

===== MANDATORY OUTPUT REQUIREMENT =====
Output a **CONCISE IMAGE EDITING PROMPT (≤300 words)** for direct use in image models. The prompt must be a natural language description of the final edited image, including:
- Only the final central image preservation/redesign instruction (no reasoning).
- Top/bottom grey area design (texture/pattern, color, in natural language).
- Hierarchical selling point placement (primary top/large, secondary bottom/small, both in double quotes).
- Fixed face protection phrase.
- No technical/procedural language.

===== EXAMPLE (INPUT → OUTPUT) =====
**Input:** Pre-expanded 3:4 canvas (central skincare product photo with entire solid white background, top/bottom grey areas), Primary Selling Point: "Tanpa alkohol & minyak", Secondary Selling Point: "Dengan Vitamin C"
**Output Prompt:** 3:4 e-commerce image edit. Redesign the central product photo’s background to a soft pale lemon-to-warm-grey radial gradient (preserve all foreground elements 100% intact). Keep the face in the original image clear and do NOT generate any new faces. Transform the top flat grey area into a matching soft pale lemon-to-warm-grey radial gradient with a very low-opacity citrus-slice watermark and subtle paper grain texture. Place "Tanpa alkohol & minyak" as a single continuous line in the top grey area only, large bold sans-serif display font, warm orange color with a faint dark drop shadow, centered, no line breaks, not overlapping the central image. Design the bottom grey area as a darker warm-grey to pale yellow gradient with a fine linen texture. Place "Dengan Vitamin C" as a single continuous line in the bottom grey area only, one font size smaller, bold sans-serif, pale yellow color, centered, no line breaks, not overlapping the central image.
"""
...
def send_request(item):
    ...
    image = add_percentage_padding(image, padding=[0.2, 0.13, 0, 0])
    selling_points = json.loads(item["selling_points"])

    # 1. prompt生成
    meta_prompt = poster_pip_add_text.format(first_selling_points=selling_points[0], second_selling_points=selling_points[1])
    prompt_bg = gpt_client.make_image_request("", meta_prompt, [], [], image_pils=[image], max_tokens=5000, timeout=60)
