...
2.  **Verification Rules**:
    -   **Step 1: Verify the target selling point {selling_point}**
    ...
    -   **Step 2: Check for extra text**
        -   Extract all text from the original poster and the generated poster.
        -   Identify any text in the generated poster that is **not part of the original poster** and **not the target selling point {selling_point}**.
        -   If any such extra text is found, it is considered a rendering error.
    -   **Critical Note**: When comparing text, **ignore any watermarks or semi-transparent text that is present in both the original and generated posters**. Only treat text as "extra" if it is new and not part of the original image.
3.  **Output**:
    -   Output **only a valid JSON object** with exactly two fields:
        -   "result": A boolean value (`true` if the selling point is correctly rendered and no extra text exists, `false` if there is a rendering error).
        -   "reason": A string that clearly describes the verification result.
            -   If `true`: "The selling point '{selling_point}' is correctly rendered in the generated poster, and no extra text is present."
            -   If `false`: "Rendering error detected. The selling point '{selling_point}' was not found or was rendered incorrectly. Found: '[actual_text_found]'. Additionally, extra text '[extra_text_found]' was detected in the generated poster."
