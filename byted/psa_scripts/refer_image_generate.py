...
def parse_args():
    ...
    parser.add_argument(
        "--input_file", default="/mlx_devbox/users/xushangliang/playground/pro_psa_feed_0318_online_cost.csv", type=str)
    parser.add_argument(
        "--refer_file", default="refer_100_psa_product_info.csv", type=str)
    parser.add_argument(
        "--output_file", default="result_pro_psa_feed_0318_online_cost.csv", type=str)
...
# Core Objective
Create a premium, commercially usable e-commerce image that preserves the true source product while upgrading the scene, atmosphere, and presentation.

In addition to environment and styling transfer, you may also transfer the reference image's presentation pattern only when it is clearly visible and appropriate for the source product.
...
- like the example
- following the style of another image
- hidden design process
- internal analysis

Describe only the final visible result and editing intent.

# Absolute Rule 1B: Environment-Level Cues May Be Transferred from the Reference
From the reference image, you may extract environment-level and presentation-level cues such as:
- background style
- surface or platform design
- lighting direction and softness
- shadow quality
- depth of field
- spatial mood
...
These cues may be used in the final prompt.

# Absolute Rule 1C: Presentation Pattern Transfer Is Allowed Only If Clearly Supported by the Reference
You may transfer the reference image's presentation pattern only if that pattern is clearly and explicitly shown in the reference image.

Examples of eligible presentation patterns include:
- try-on presentation
- product being worn
- product being carried
- product being held
- close-up body-part framing
- one item in use plus another item shown separately
- partial human presence used to demonstrate fit, scale, or real-life usage

If the reference image does not clearly show such a pattern, do not invent it.

# Absolute Rule 1D: Never Transfer Product-Specific Details from the Reference
Do not transfer any product-level specifics from the reference, including:
- product category
- subtype
- silhouette
...
If a detail belongs to the reference product rather than the surrounding scene or presentation logic, it must not appear in the final prompt.
...
# Absolute Rule 2C: Human Presence Must Be Functional, Not Inventive
If you introduce human presence, it must serve only as a presentation device for the source product.

Human presence may be used only when all of the following are true:
1. the reference image clearly shows human involvement or a try-on / in-use presentation pattern
2. the source product reasonably supports that same kind of presentation
3. the resulting composition remains product-led and commercially practical

Human presence must remain limited, cropped, and utility-driven when possible, such as:
- a foot
- a hand
- a wrist
- a shoulder
- a partial leg
- a partial arm
- a close-up cropped body area

Do not invent full character scenes, emotional narratives, fashion-editorial storytelling, or unnecessary body exposure.

# Absolute Rule 2D: Do Not Add Human Presence When the Reference Is a Pure Object Display
If the reference image presents the product purely as an object display with no human involvement, do not introduce any human body part, try-on setup, or usage scene.

# Absolute Rule 2E: Use Try-On or In-Use Presentation Only When the Mapping Is Natural
Only use try-on, worn, carried, held, or in-use presentation if it naturally fits the source product.

Examples:
- shoes -> foot try-on may be appropriate
- bag -> shoulder carry or hand carry may be appropriate
- watch -> wrist close-up may be appropriate

Do not force a presentation pattern that feels unnatural for the source product.

# Absolute Rule 2F: Any Inferred Extra Item Must Remain Low-Risk and Consistent
If the source image shows only one item but the product is commonly understood to be part of a pair or set, you may infer an additional matching item only when this is low-risk, commercially reasonable, and consistent with the source image.

Do not invent uncertain unseen features.
...
# Absolute Rule 5: Focus the Prompt on Editing the Environment and Presentation, Not Recreating the Product
The final prompt should mainly describe:
- how the original product is preserved
- how it is placed or presented
- whether there is a clearly justified try-on or in-use presentation
- what surface or platform it sits on
- what the background environment looks like
- what lighting and shadows are used
- what depth, atmosphere, and spatial feeling are created
- how the composition feels commercially

The prompt should not spend words re-describing the product itself unless needed for preservation.

# Beta Design Intention
In this beta version, you may go beyond pure background transfer and incorporate a reference-supported presentation pattern if it improves product communication and remains realistic, restrained, and ad-usable.

However:
- do not invent unsupported concepts
- do not add human presence without clear reference evidence
- do not let the human element dominate the ad
- do not depart from plausible e-commerce presentation logic

# Goal
Write a final prompt that:
- preserves the original product from the source image
- preserves source-existing branding and layout elements correctly
- upgrades the background, atmosphere, lighting, and presentation
- optionally applies a clearly reference-supported presentation pattern
- remains self-contained and directly usable by a downstream image-to-image model
- feels premium, realistic, product-led, ad-ready, and commercially usable
- minimizes the risk of product drift and uncontrolled invention
...
# Final Prompt Style
The final prompt should be written primarily as image-editing instructions for the existing source image, combined with direct visible-result language for the new setting and, if justified, the new presentation pattern.

Use:
1. preservation/editing language for the source product and existing assets
2. direct visual result language for the environment
3. controlled presentation-pattern language only when clearly justified
...
1. preserve the source product correctly
2. preserve source-existing branding and layout assets correctly
3. preserve a realistic, product-led commercial composition
4. transfer environment and atmosphere cues from the reference
5. only then consider a clearly supported presentation-pattern transfer
...
def send_request(item):
    ...
    origin_image_label = json.loads(item["image_label"])
    if origin_image_label["subject_classification"] in cate_data:
        refer_url = [a["image_url"] for a in cate_data[origin_image_label["subject_classification"]]]
        refer_url = random.choice(refer_url)
    else:
        origin_reference_label = json.loads(item["reference_label"])
        visual_style = random.choice(origin_reference_label["visual_style"])
        refer_url = [a["image_url"] for a in style_data[visual_style]]
        refer_url = random.choice(refer_url)

    item["refer_url"] = refer_url
...
if __name__ == "__main__":
    ...
    refer_data = load_csv_or_xlsx_to_dict(args.refer_file)
    cate_data = {}
    for line in refer_data:
        image_label = json.loads(line["image_label"])
        if image_label["subject_classification"] not in cate_data:
            cate_data[image_label["subject_classification"]] = []
        cate_data[image_label["subject_classification"]].append(line)

    style_data = {}
    # for line in refer_data:
    #     reference_label = json.loads(line["reference_label"])
    #     for style_name in reference_label["visual_style"]:
    #         if style_name not in style_data:
    #             style_data[style_name] = []
    #         style_data[style_name].append(line)

    data = load_csv_or_xlsx_to_dict(args.input_file)
    data = [a for a in data if a["strategy"] not in ["addon", "poster_redraw"]]
    out = []
    num_cate = {k: 0 for k in cate_data.keys()}
    for line in data:
        image_label = json.loads(line["image_label"])
        if image_label["subject_classification"] in cate_data:
            if num_cate[image_label["subject_classification"]] < 10:
                out.append(line)
                num_cate[image_label["subject_classification"]] += 1
    print(num_cate)
