...
pipe = QwenImageT2IEditPipeline.from_pretrained(pretrained_model_name_or_path, torch_dtype=torch.bfloat16)
pipe = pipe.to(device)

pipe.load_lora_weights("/mnt/bn/creative-algo/xsl/exp_weights/gen_ai_100e/pytorch_lora_weights.safetensors")
pipe.fuse_lora(lora_scale=1.0)
pipe.unload_lora_weights()

out_dir = "gen_t2i_lora"
os.makedirs(out_dir, exist_ok=True)

row_dict = load_file("/mnt/bn/creative-algo/xsl/data/gen_ai_dataset/test.json")

for line in tqdm(row_dict):
    product_img = load_or_download_image(line['product_img'])
    product_img = product_img.resize((640, 640), 1)
    prompt = line['prompt']
    out_img = pipe(
