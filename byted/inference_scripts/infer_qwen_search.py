...
def parse_args():
    ...
    parser.add_argument(
        "--save_dir",
        default="/mnt/bn/creative-algo/xsl/qwen_search",
        type=str,
    ...
    parser.add_argument(
        "--transformer_path",
        type=str,
        default="/mnt/bn/creative-algo/xsl/xsl-subject-1017/checkpoint-30000",
    )
    parser.add_argument(
        "--lora_model_path",
        type=str,
        default="/mnt/bn/creative-algo/xsl/xsl-search-1023/checkpoint-20000/pytorch_lora_weights.safetensors",
    )
...
def main(args):
    ...
    transformer = QwenImageTransformer2DModel.from_pretrained(
        args.transformer_path, torch_dtype=dtype)
    transformer.requires_grad_(False)
    transformer.to(device, dtype=dtype)
    # Load pipeline
    pipe = QwenImageEditPipeline.from_pretrained(args.pretrained_model_path, transformer=transformer, torch_dtype=dtype)
    pipe = pipe.to(device)
    # Load lora
    ...
    for line in tqdm(data_list):
        try:
            product_img = load_or_download_image(line['product_image'])
            prompt = line['prompt']
            out_img = pipe(
                ...
                true_cfg_scale=4.0,
                reference_image_size=(1024, 1024),
            ).images[0]
